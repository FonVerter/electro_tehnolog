...
def calculate_cutting_parameters(dxf_file):
    ...
    # Список для хранения всех точек врезки
    cutting_points = []
    ...
    for circle in msp.query('CIRCLE'):
        ...
        cutting_points.append(center)  # Точка врезки — центр окружности
    ...
    for ellipse in msp.query('ELLIPSE'):
        ...
        cutting_points.append(center)  # Точка врезки — центр эллипса
        ...
        for start, end in lines:
            if start not in visited:
                ...
                if len(polygon) > 2:  # Многоугольник должен иметь хотя бы 3 вершины
                    ...
                    total_area += area
                    cutting_points.append(polygon[0])  # Точка врезки — первая вершина многоугольника

    # Вывод результатов
    print(f"Общая длина реза: {total_length:.2f} единиц")
    print(f"Общая площадь для вырезания: {total_area:.2f} квадратных единиц")
    print(f"Количество точек врезки: {len(cutting_points)}")
    print("Точки врезки:", cutting_points)
...
calculate_cutting_parameters("C:\\Users\\FV\\Desktop\\python\\dxf-analyzer\\squareCircle.DXF")

#считает верно. точкки врезки неверные